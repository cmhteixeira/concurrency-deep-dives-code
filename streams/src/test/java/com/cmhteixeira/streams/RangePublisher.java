...
/**
 * A synchronous implementation of the {@link Publisher} that can be subscribed to multiple times
 * and each individual subscription will receive range of monotonically increasing integer values on
 * demand.
 */
public final class RangePublisher implements Publisher<Integer> {
  ...
  /**
   * Constructs a RangePublisher instance with the given start and count values that yields a
   * sequence of [start, start + count).
   *
  ...
  /**
   * A Subscription implementation that holds the current downstream requested amount and responds
   * to the downstream's request() and cancel() calls.
   */
  static final class RangeSubscription
    ...
    /**
     * The current index and within the [start, start + count) range that will be emitted as
     * downstream.onNext().
     */
    int index;
    ...
    /**
     * Holds onto the IllegalArgumentException (containing the offending stacktrace) indicating
     * there was a non-positive request() call from the downstream.
     */
    volatile Throwable invalidRequest;

    /**
     * Constructs a stateful RangeSubscription that emits signals to the given downstream from an
     * integer range of [start, end).
     *
    ...
    public void request(long n) {
      ...
      if (n <= 0L) {
        invalidRequest =
            new IllegalArgumentException("§3.9: non-positive requests are not allowed!");
        n = 1;
      }
      ...
      try {
        for (; ; ) {
          ...
          if (invalidRequest != null) {
            // When we signal onError, the subscription must be considered as cancelled, as per rule
            // 1.6
            cancelled = true;
        ...
        (new IllegalStateException(
                downstream
                    ...
                    + "throwing an exception from onNext, onError or onComplete.",
                ex))
            .printStackTrace();
      }
    }
