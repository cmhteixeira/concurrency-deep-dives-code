...
public class BinaryQueueTest {
    ...
    public void foo() {
        ...
        Assert.assertEquals(fooo.findMin().ttr, 13);
    }
    ...
    public void bar() {
        ...
        Assert.assertEquals(fooo.findMin().ttr, 4);
    }
    ...
    public void baz() {
        ...
        Assert.assertEquals(queue.popMin().ttr, 1);
        Assert.assertEquals(queue.popMin().ttr, 2);
        Assert.assertEquals(queue.popMin().ttr, 3);
        Assert.assertEquals(queue.popMin().ttr, 4);
//        int randomWithMathRandom = (int) ((Math.random() * (max - min)) + min);
//        Assert.assertEquals(fooo.popMin().ttr, 10);
    }
...
//        Assert.assertEquals(myresult, theirResult);
        int j = 1;
        while (j <= iter2) {
            var theirResult =  prioQueue.poll().ttr;
            System.out.println(String.format("Min is: %d", theirResult));
            Assert.assertEquals(queue.popMin().ttr,theirResult);
            ++j;
        }
    }
}
